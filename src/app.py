...
def tracking(frame: np.ndarray, pupil_detection_mode: str):
...
def init(video=False, pupil_detection_mode="grad_means", image_path=os.path.join('resources','images','face2.jpg')):
    ...
    if video:
        ...
        frame = tracking(frame, pupil_detection_mode)
        cv2.imwrite(os.path.splitext(image_path)[0] + '_edited' + os.path.splitext(image_path)[1], frame)
        cv2.waitKey()
