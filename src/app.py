...
from components.pupil_detection_means_gradients import PupilDetection
from components.pupil_detection_filtering import PupilDetection as PupilDetection2
...
def tracking(frame, pupil_detection_mode):
    ...
    for face in landmark_tracking.face_analysis(framebg):
        ...
        pupil_detection_modes = {
            "grad_means": pupil_detection, 
            "filtering": pupil_detection_2
        }
        
        pupil_sx_x, pupil_sx_y = pupil_detection_modes[pupil_detection_mode].detect_pupil(eyes[0])
        pupil_dx_x, pupil_dx_y = pupil_detection_modes[pupil_detection_mode].detect_pupil(eyes[1])
        ...
        pupil_sx_center_h_ratio = round((pupil_sx_x - face.get("eye_sx_in")[0]) / (
            face.get("eye_sx_out")[0] - face.get("eye_sx_in")[0]) - 0.5, 2)
        pupil_dx_center_h_ratio = round((pupil_dx_x - face.get("eye_dx_out")[0]) / (
            face.get("eye_dx_in")[0] - face.get("eye_dx_out")[0]) - 0.5, 2)
        gaze_facing = False
...
def init(video=False, pupil_detection_mode="grad_means", image_path='images/face1.png'):
    ...
    if video:
        ...
        while(True):
            ...
            tracking(frame, pupil_detection_mode)
